...
implement trans_clo1_v1aldeclst_rec (valdecs, Gamma) = let
  ...
  fun closure_collect_env_e1xp (init: ctx, e: e1xp):<cloref1> ctx =
    case+  e.e1xp_node of
    | E1XPann (e1, _) => closure_collect_env_e1xp (init, e1)
    | E1XPfix (_, _, _, env_ref) => let
      val env = !env_ref
      val env = list0_map_fun<v1ar>< @(symbol_t, v1ar)> (
          env, lam x => @(x.v1ar_nam, x))
      val new_env = symenv_inserts (init, env)
    in
      new_env
    end
  ...
  fun closure_collect_env (init: ctx, valdec: v1aldec):<cloref1> ctx =
    closure_collect_env_e1xp (init, valdec.v1aldec_def)
    
  val env_all = list0_fold_left<ctx><v1aldec> (
               closure_collect_env, ctx_nil, valdecs)
  val env_all = symenv_sub (env_all, vars_ctx)
  val env_all = list0_map_fun< @(symbol_t, v1ar)><v1ar> (
           symenv_listize (env_all), lam x => x.1)
